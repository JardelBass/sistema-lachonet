<div class="col s3 admin-p">
    <div class="card-panel admin-lc red lighten-2">
        <span class="white-text text-darken-2">LISTA DE PEDIDOS</span>
    </div>
    ...
    if($vendaContar != 0){
        ...
        ?>
        <a href="?pagi=admin&&id=<?= $listaVenda->sale_code?>" class="collection-item black-text " id="<?= $listaVenda->sale_code?>"><i class="material-icons right">send</i>MESA: <?= $listaVenda->sale_table; ?></span></a>   
        <?php }}}else{ ?>
...
<div class="col s8 offset-s4 admin-d">
    <?php if(isset($_GET['id'])){ $id = $_GET['id']; ?>
        ...
        <?php
            ...
            while($listaVenda = $venda->FETCH(PDO::FETCH_OBJ)){
                if($listaVenda->sale_code == $codigo){
                    $stato = $listaVenda->sale_table;
                    $id = $listaVenda->sale_id;
        ?>
                <table class="striped">
                    <thead>
                        <tr>
        ...
        <?php if($stato == "ENTREGA" || $stato == "VIAGEM"){ ?>
        <br/>
            <a class="waves-effect waves-light btn red col s12" href="?pagi=admin&&final=<?= $id ?>">FINALIZAR PEDIDO</a>
        <?php }?>
    </div>
    <?php } ?>
</div>
